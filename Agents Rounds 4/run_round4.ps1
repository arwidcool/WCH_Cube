<#
  ...
  access to. Every session starts with permission prompts disabled.
      ...
      pwsh -File "Agents Rounds 4\run_round4.ps1"           # start all four
      pwsh -File "Agents Rounds 4\run_round4.ps1" -Only 2   # start just AGENT-2
      pwsh -File "Agents Rounds 4\run_round4.ps1" -DryRun   # print the commands, start nothing
#>
[CmdletBinding()]
param(
...
$pack = Join-Path $root 'Agents Rounds 4'
...
Read, in this order, before you touch anything:
  ...
  2. Agents Rounds 4/00_PROJECT.md          the round brief
  3. Agents Rounds 4/$($a.File)             your standing instructions
  4. PROGRESS.md                            where the project actually stands
  5. TASKS.md                               the backlog - claim with [~] (AGENT-$($a.N))
  6. Agents Rounds 4/BOARD.md, then the last entries of Agents Rounds 3/BOARD.md
...
Stop when Agents Rounds 4/BOARD.md contains 'ROUND 4 DONE'.
"@
